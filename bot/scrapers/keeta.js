...
async function search(restaurantName) {
  return withBrowser(async (ctx) => {
    ...
    const results = await page.evaluate(() => {
      const cards = document.querySelectorAll('[class*="vendor"], [class*="restaurant"], [class*="store"]');
      return Array.from(cards).slice(0, 5).map(card => ({
        name: card.querySelector('[class*="name"], h2, h3, strong')?.textContent?.trim() || '',
...
async function getMenu(restaurantUrl) {
  return withBrowser(async (ctx) => {
    ...
    const menu = await page.evaluate(() => {
      const categories = [];
      const catEls = document.querySelectorAll('[class*="category"], [class*="section"], [class*="group"]');

      if (catEls.length === 0) {
        const items = document.querySelectorAll('[class*="item"], [class*="product"], [class*="meal"]');
      ...
      catEls.forEach(cat => {
        ...
        const items = Array.from(cat.querySelectorAll('[class*="item"], [class*="product"]')).map(el => {
