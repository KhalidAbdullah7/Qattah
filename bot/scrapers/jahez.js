...
async function getMenu(restaurantUrl) {
  return withBrowser(async (ctx) => {
    ...
    const menu = await page.evaluate(() => {
      const categories = [];
      const catEls = document.querySelectorAll('[class*="category"], section, [class*="menu-section"], [data-testid*="section"]');

      if (catEls.length === 0) {
        const items = document.querySelectorAll('[class*="product"], [class*="item"], [class*="meal"], [data-testid*="item"]');
      ...
      catEls.forEach(cat => {
        ...
        const items = Array.from(cat.querySelectorAll('[class*="product"], [class*="item"], [data-testid*="item"]')).map(el => {
          const name = el.querySelector('[class*="name"], h3, h4')?.textContent?.trim();
          const priceText = el.querySelector('[class*="price"]')?.textContent?.trim();
          const price = priceText ? parseFloat(priceText.replace(/[^\d.]/g, '')) : 0;
          return name && price > 0 ? { name, price } : null;
